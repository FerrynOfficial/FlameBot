...
exports.run = (client, message, args) => {
    ...
    var modcmds = ['`warn` - Выдает предупреждение', '`kick` - Кикает участника', '`ban` - Банит участника', '`purge` - Очищает чат', '`mute` - Выдает мут', '`dm` - Отправляет личное сообщение участнику']
    var cmds = ['`ping` - Показывает пинг бота', '`poll` - Проводит голосование', '`serverinfo` - Показывает информацию о сервере', '`about` - Показывает информацию о боте', '`report` - Пожаловаться на участника', '`open-case`- Открыть донат кейс', '`userinfo`- Получить информацию о участнике']
